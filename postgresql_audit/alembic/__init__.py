...
def compare_timestamp_schema(autogen_context, upgrade_ops, schemas):
    ...
    for sch in schemas:
        ...
        a = next(
            (v for k, v in groupby(routines, key=lambda x: x[0]) if k == sch), None
        )
        a = list(a) if a else []
        if should_track_versions:
            if f"{schema_prefix}audit_table" not in (x[1] for x in a):
                upgrade_ops.ops.insert(
                    0, InitActivityTableTriggersOp(False, schema=sch)
                )
        else:
            if f"{schema_prefix}audit_table" in (x[1] for x in a):
                upgrade_ops.ops.append(RemoveActivityTableTriggersOp(False, schema=sch))
...
def compare_timestamp_table(
    ...
    if "versioned" in meta_info:
        ...
        trigger = next(
            (trigger for trigger in triggers if trigger_name in trigger[3]), None
        )
        ...
        if trigger:
            modify_ops.ops.append(
                DeregisterTableForVersionTrackingOp(
                    tablename, original_excluded_columns, schema=schema_name
                )
            )
...
def __get_existing_excluded_columns(trigger):
    ...
    if trigger:
        arguments_match = re.search(
            r"EXECUTE FUNCTION create_activity\('{(.+)}'\)", trigger[7]
        )
        if arguments_match:
